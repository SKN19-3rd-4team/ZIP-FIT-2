        ...
        types.forEach((type, typeIdx) => {
            ...
            statusVariations.forEach((statusVar, statusIdx) => {
                ...
                announcements.push({
                    ...
                    "annc_url": url,
                    "created_dttm": createdDate.toISOString(),
...
// ============================================
// 4. GET /api/chathistories/{session_key} Mock 데이터
// ============================================
// 다양한 채팅 히스토리 시나리오
...
const MockAPI = {
    ...
    async getAnnouncements(params = {}) {
        ...
        if (params.annc_status && params.annc_status !== '' && params.annc_status !== '전체') {
            items = items.filter(item => {
                const createdDate = item.created_dttm ? new Date(item.created_dttm) : today;
                const startDate = new Date(createdDate);
                startDate.setDate(startDate.getDate() + 1);
                const endDate = new Date(startDate);
     ...
     */
    async getChatHistoryDetail(sessionKey, userKey) {
        await new Promise(resolve => setTimeout(resolve, 500));
        
        // 시나리오별로 다른 채팅 히스토리 반환
        if (typeof mockChatHistoryScenarios !== 'undefined' && mockChatHistoryScenarios[sessionKey]) {
            return {
                "message": "성공적으로 특정 채팅 히스토리를 조회했습니다.",
                "status": "success",
                "data": mockChatHistoryScenarios[sessionKey]
            };
        }
     ...
     * 채팅 메시지 전송 (Mock)
     */
    async sendChatMessage(userMessage, userKey, sessionKey) {
        ...
        return {
            ...
            data: {
                ai_response: {
                    ...
                    session_id: sessionKey || 'mock-session-' + Date.now()
                }
            }
        };
    }
...
const API = {
    ...
    async getChatHistoryDetail(sessionKey, userKey) {
        if (USE_MOCK_DATA) {
            return await MockAPI.getChatHistoryDetail(sessionKey, userKey);
        } else {
            // 실제 API 호출: api.js의 getChatHistoryDetail은 { data: { chat_list: [...] } } 형태 반환
            try {
                const response = await getChatHistoryDetail(sessionKey, userKey);
                // api.js의 getChatHistoryDetail은 data.data를 반환하므로, 이를 표준 형식으로 래핑
                // 실제 API 응답 구조: { data: { title, session_key, user_key, chat_list: [...] } }
                return {
                    status: 'success',
                    data: response || {}
                };
    ...
    async sendChatMessage(userMessage, userKey, sessionKey) {
        if (USE_MOCK_DATA) {
            return await MockAPI.sendChatMessage(userMessage, userKey, sessionKey);
        } else {
            // 실제 API 호출: api.js의 sendChatMessage는 { data: { ai_response: {...} } } 형태 반환
            try {
                const response = await sendChatMessage(userMessage, userKey, sessionKey);
...
async function callApi(endpoint, method = 'GET', body = null) {
    if (USE_MOCK_DATA) {
        ...
        if (endpoint === '/api/annc_summary') {
            ...
            // 특정 채팅 히스토리 상세 조회
            const sessionKey = endpoint.split('/').pop().split('?')[0];
