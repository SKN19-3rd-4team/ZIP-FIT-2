        ...
        if elem.element_type == 'heading':
            ...
            for tc in chunk_table(elem.content, context):
                if is_valid_chunk(tc, min_size=30):
                    ...
                    chunks.append(Chunk(tc, chunk_index, elem.page_number, 'table', final_context, {'document_id': document_id}))
                    chunk_index += 1
            current_heading = None
            ...
            for tc in split_text_into_chunks(text):
                if is_valid_chunk(tc):
                    chunks.append(Chunk(tc, chunk_index, elem.page_number, 'text', None, {'document_id': document_id}))
                    chunk_index += 1
