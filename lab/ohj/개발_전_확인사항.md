...
### 1. 프로젝트 구조
- ✅ `zipfit_django` 구조를 따라야 함
- ✅ `figma_django` 폴더에 새로 구성
- ✅ 5개 페이지: `landing`, `user_info`, `main`, `chat`, `list`
- ✅ API 호출 방식 (models 사용 안 함)

### 2. 기술 스택
- ✅ Django 템플릿 시스템 사용
- ✅ 세션 기반 사용자 정보 저장
- ✅ PostgreSQL (RDB + VectorDB)
- ✅ Docker 환경

### 3. API 문서 확인
- ✅ `docs/api/api.yaml` 확인 완료
- ✅ 채팅 API: `/api/chat` (POST)
- ✅ 공고 목록 API: `/api/anncs` (GET)
- ✅ 통계 API: `/api/annc_summary` (GET)
- ✅ 채팅 히스토리 API: `/api/chathistories` (GET)
...
- 하지만 `api.yaml`에는 `/api/chat`로 정의되어 있음
